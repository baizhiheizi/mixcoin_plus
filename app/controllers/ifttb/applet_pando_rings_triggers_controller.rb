...
class Ifttb::AppletPandoRingsTriggersController < Ifttb::BaseController
  before_action :load_applet
  before_action :load_trigger, only: %i[edit update destroy]
  before_action :load_assets, only: %i[new edit]

  def new
    @trigger = @applet.applet_triggers.find_by(type: 'AppletPandoRingsTrigger')
    @trigger ||= @applet.applet_triggers.new(type: 'AppletPandoRingsTrigger')
    redirect_to edit_ifttb_applet_applet_PandoRings_trigger_path(@applet, @trigger) if @trigger.persisted?
  end
  ...
  def create
    @trigger = AppletPandoRingsTrigger.new trigger_params
  ...
  def load_assets
    @mixin_assets =
      ...
      .where(asset_id: PandoRings.api.supportable_asset_ids)
      .order(symbol: :asc)
  end

  def trigger_params
    params
      .require(:applet_pando_rings_trigger)
      .permit(
        :type,
        :applet_id,
        :asset_id,
