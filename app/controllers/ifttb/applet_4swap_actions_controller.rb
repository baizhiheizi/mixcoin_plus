...
class Ifttb::Applet4swapActionsController < Ifttb::BaseController
  before_action :load_applet
  before_action :load_action, only: %i[edit update destroy]
  before_action :load_assets, only: %i[new edit]

  def new
    @action = @applet.applet_actions.find_by(type: 'Applet4swapAction')
    @action ||=
      @applet
      .applet_actions
      .new(
    ...
    redirect_to edit_ifttb_applet_applet_4swap_action_path(@applet, @action) if @action.persisted?
  end
  ...
  def create
    @action = Applet4swapAction.new action_params
