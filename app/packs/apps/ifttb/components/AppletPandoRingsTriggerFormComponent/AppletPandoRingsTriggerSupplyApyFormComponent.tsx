...
export default function AppletPandoRingsTriggerSupplyApyFormComponent(props: {
  ...
  const appletPandoRingsTrigger = appletForm?.appletTriggersAttributes?.find(
    (trigger) =>
      ...
      ['supply_apy'].includes(trigger.params?.targetIndex),
  );
  ...
  const createTrigger = () => {
    const trigger = {
      ...
      params: {
        description: `Supply APY of ${asset.symbol} (from Pando Rings) ${
          compareAction === 'larger_than' ? '>=' : '<='
        } ${targetValue}%`,
        assetId: asset?.assetId,
        targetIndex: 'supply_apy',
  ...
  return (
    <div className='p-4'>
      <div className='mb-8 text-lg'>
        <div className='mb-4'>Run your applet when supply APY of</div>
        <div
