...
export default function AppletPandoRingsTriggerBorrowVolumeFormComponent(props: {
  ...
  const appletPandoRingsTrigger = appletForm?.appletTriggersAttributes?.find(
    (trigger) =>
      ...
      ['borrow_volume'].includes(trigger.params?.targetIndex),
  );
  ...
  const createTrigger = () => {
    const trigger = {
      ...
      params: {
        description: `Borrow Volume of ${asset.symbol} (from Pando Rings) ${
          compareAction === 'larger_than' ? '>=' : '<='
        } ${targetValue}`,
        assetId: asset?.assetId,
        targetIndex: 'borrow_volume',
  ...
  return (
    <div className='p-4'>
      <div className='mb-8 text-lg'>
        <div className='mb-4'>Run your applet when borrow volume of</div>
        <div
