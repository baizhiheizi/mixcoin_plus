...
# update load path so that the gem can find libsecp256k1
RUN ldconfig
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y imagemagick libvips postgresql-client redis libsodium-dev && \
    rm -rf /var/lib/apt/lists /var/cache/apt/archives
